...
<template>
  <section>
    <div class="{max-w:screen-max;mx:auto;overflow:hidden} p:10x|6x px:10x@desktop text:center">
      ...
      <div v-if="news?.length" class="{flex;flex:col;gap:5x} {max-w:screen-main;mx:auto} mt:5x px:5x@desktop">
        <NewsCard v-for="i in news.slice(0, 3)" :key="i.ID" class="bg:home!" :news="i" />
      </div>
