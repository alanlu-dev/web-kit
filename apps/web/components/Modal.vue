...
<template>
  <VueFinalModal
    ...
    :content-class="['{t:left}_:where(.vfm__content__content) bg:base-bg max-h:calc(100vh-30) max-w:360 overflow:hidden pb:6x r:2x shadow:xl t:center w:calc(100vw-30)', localProps.contentClass]"
  ...
  >
    <template #default="{ close }">
      <div class="vfm__content__root rel mt:6x px:6x w:full">
        ...
        <header class="vfm__content__header {hidden;mb:0}:empty mb:6x user-select:none">
          <slot name="header">
        ...
        <div class="vfm__content__content flex">
          <slot>
