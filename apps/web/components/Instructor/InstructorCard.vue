  ...
  <div class="{block;content:'';h:1;bg:divider;w:full;my:5x}::after {my:10x}::after@xs {hidden}:last::after my:5x">
    <div class="{flex;flex:col} {flex:row;gap:10x}@xs">
      ...
      <div class="flex:2">
        ...
        <div class="accordion-content {accordion-content--open}@xs">
          <div>
            <div class="b1-m bg:home fg:primary mt:4x mt:0@xs p:5x|6x r:5x">
              {{ instructor.標語 }}
            </div>

            <div v-if="instructor.專業認證?.length" class="mt:4x mt:7x@xs">
              <p class="b1-m flex rel {abs;middle;left:0;content:'';w:1.5x;bg:font-title/.2}::before fg:font-title pl:3.5x"> 專業認證</p>
              <div class="list b1-r mt:2x mt:3x@xs pl:0.5x@xs">
                <ul>
            ...
            <div v-if="instructor?.服務經驗" class="mt:4x mt:7x@xs">
            ...
            <div v-if="instructor.受邀講座?.length" class="mt:4x mt:7x@xs">
        ...
        <label :for="`i-${instructor.ID}`" class="block hidden@xs :checked~{block!}>.close :checked~{hidden}>.open mt:2x text:center">
